...
本项目逐步向**企业级通用agent助手**演进，基于Plan-Execute、React、self-Reflection范式编排：
...
* **黑板驱动 (`PLAN.md`)**：主节点规划与派发、子节点执行。一切行动基于物理落盘的DAG任务图。
* **最终审查机制**：Agent 试图提交任务前，会被强制打断并进行 `Reflection`，保证任务最终完成。
